const API_BASE_URL = "https://rshea42--overwatch-hero-recommender-fastapi-app.modal.run";
...
function populateHeroSelects() {
  const sorted = [...DAMAGE_HEROES].sort((a, b) => a.name.localeCompare(b.name));
  ...
  // Default starting hero, matching the already-verified test case.
...
async function handleSubmit(event) {
  ...
  if (!hero1) {
    showFormError("Please select a starting Damage hero.");
    return;
  }
